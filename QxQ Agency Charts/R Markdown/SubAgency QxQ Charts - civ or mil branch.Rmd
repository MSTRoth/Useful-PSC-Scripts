---
title: 'Creating PSC Market Briefing Charts: Subagency Charts - QxQ - Civilian or Military Branch'
...
will make ALL subagency charts and save them - note that some will be incomplete or unusable.  Just delete these, but it should make all the ones you might need for a briefing


## Quarter by Quarter Agency Charts
### Civilian Agencies or Military Branches

<!-- This script will produce the quarter by quarter stacked bar chart for Civilian Agencies or Departments or Military Branches.  It won't work for DoD because the data pull is too big to download from BGOV.  See the **Agency Charts - QxQ - DoD** markdown file for creating the DoD quarter by quarter chart. -->
...
data.organized<- lapply(Subagency, function(x){
  data.organized <- data %>%
    ...
    mutate(total_obligations = round((sum)/1000000, digits=2)) %>%           
...
Additionally, if the subagencies have particularly large or small value contracts, the 1000000 can be changed to 1000000000 or 1000 make the plot easier to read. If you change it, keep it in mind when creating the plot below to change Millions to Billions or Thousands.  This would be changed in the line

     mutate(total_obligations = round((sum)/1000000, digits=2)) %>%


Next, create the barplots. Run the following:
```{r, eval=FALSE}
...
The plot should appear in the window at the bottom right.  If it doesn't, make sure you ran all the lines above and that there are no typos in your changes.  To see it larger, push the **Zoom** button.  To view each chart, use the arrows in the bottom right window to scroll through.  Some will probably be unreadable or lack content.  These can be deleted from the file afterwards.
