---
title: "Products vs Services Quarter by Quarter Charts - Civilian and Military Branch"
...
This script will produce the products vs services quarter by quarter stacked bar chart for Civilian Agencies/Departments or Military Branches.  It won't work for DoD because the data pull is too big to download from BGOV.  See the **Agency P-S QxQ Charts - DoD** markdown file for creating the DoD quarter by quarter chart.
...
After saving and renaming the file, open the script **Agency P-S QxQ Charts - civ or mil branch** located in **X:\1 Marielle Folder\R Scripts\Useful PSC Scripts\QxQ Agency Charts**
...
Then define the name of the Agency and Year in quotes, using the exact name and formatting used to save the file. See the following example (for a file saved as **USAID FY17-19Q3.csv**):
...
Next read the necessary two datasets into R by running the following, and set the working directory:
...
data.organized <- data %>% 
  ...
  filter(PSC != "UNKN") %>% 
  filter(fiscal_year %in% c(2017:2019)) %>% 
  select(transaction_value, fiscal_quarter, `P.S`) %>% 
  group_by(`P.S`, fiscal_quarter) %>%    
...
plot <- 
  ...
  facet_wrap(~`P.S`, labeller = label_wrap_gen(20)
  )+
...
```{r, eval=FALSE}
ggsave(filename = paste(Agency," Contract Obligations ", dis_year, "by quarter - P-S.jpg", sep = ""), plot,          
